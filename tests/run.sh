...
if [ $# -eq 0 ]; then
    py.test $XDIST_ARGS $MAX_FAIL_ARG -s --tb=short --verbose --junitxml=results.xml $HTML_REPORT --runfast --runslow tests/
    exit $?
fi

py.test $XDIST_ARGS $MAX_FAIL_ARG -s --tb=short --verbose --junitxml=results.xml $HTML_REPORT "$@" tests/
